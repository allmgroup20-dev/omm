    ...
    <div className="min-h-screen grid place-items-center bg-zinc-50 p-4 sm:p-6">
      <div className="w-full max-w-md bg-white border rounded-[24px] p-6 sm:p-8 shadow-sm">
        ...
        <form onSubmit={submit} className="mt-6 space-y-4">
          <input type="email" placeholder={t("auth.email")} value={email} onChange={(e) => setEmail(e.target.value)} className="w-full rounded-xl border px-4 py-3 text-base sm:text-sm outline-none focus:ring-2 focus:ring-zinc-900 min-h-[44px]" required />
          <button className="w-full rounded-full bg-zinc-900 text-white py-3 text-sm font-medium min-h-[44px]">{t("auth.sendLink")}</button>
        </form>
