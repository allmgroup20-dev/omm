  ...
  return (
    <div className="min-h-screen grid place-items-center bg-zinc-50 p-6">
      <div className="w-full max-w-md bg-white border rounded-[24px] p-8 shadow-sm">
        ...
        <div className="mt-6">
          <GoogleButton text={t("auth.googleContinue")} />
        </div>
        <div className="mt-4">
        ...
        <div className="mt-4 flex justify-between text-sm">
          ...
          <Link href="/register" className="font-medium text-zinc-900 underline">{t("nav.register")}</Link>
        </div>
        <p className="mt-4 text-center">
          <Link href="/" className="text-xs text-zinc-500 hover:text-zinc-900">{t("common.goHome")}</Link>
        </p>
      </div>
    </div>
  );
...
export default function LoginPage() {
  return (
    <Suspense fallback={<div className="min-h-screen grid place-items-center bg-zinc-50 p-6">...</div>}>
      <LoginInner />
    </Suspense>
  );
}
