...
export async function POST(req: Request, { params }: { params: Promise<{ id: string; expenseId: string }> }) {
  ...
  if (!access[0] || access[0].role !== "manager") return NextResponse.json({ error: "Only manager can reject" }, { status: 403 });
  ...
  await db.update(expenses).set({ status: "rejected", updatedAt: now }).where(eq(expenses.id, expenseId));
  await db.insert(expenseApprovals).values({ id: nanoid(), expenseId, approverId: user.id, status: "rejected", note: parsed.data.note?.trim() || null, createdAt: now });
  await db.insert(auditLogs).values({ id: nanoid(), messId: id, actorId: user.id, action: "reject", entityType: "expense", entityId: expenseId, afterJson: JSON.stringify({ status: "rejected" }), createdAt: now });
