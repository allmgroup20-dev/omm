...
export default function EntryDetailPage() {
  ...
  const [form, setForm] = useState({ date: "", classification: "food", paymentMethod: "cash", discount: "0", transport: "0", notes: "" });
  ...
  async function load() {
    ...
    if (res.ok) {
      setEntry(data.entry);
      setItems(data.items);
      setForm({ date: data.entry.date, classification: data.entry.classification, paymentMethod: data.entry.paymentMethod, discount: (data.entry.discountPaisa / 100).toString(), transport: ((data.entry.transportPaisa || 0) / 100).toString(), notes: data.entry.notes || "" });
    } else setMsg(data.error);
  }
  useEffect(() => { load(); }, [id, entryId]);

  async function save() {
    const payload: Record<string, unknown> = { date: form.date, classification: form.classification, paymentMethod: form.paymentMethod, discount: parseFloat(form.discount) || 0, transport: parseFloat(form.transport) || 0, notes: form.notes };
