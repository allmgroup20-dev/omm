...
type Entry = { id: string; date: string; vendorId: string | null; classification: string; paymentMethod: string; totalPaisa: number; discountPaisa: number; finalPaisa: number; notes: string | null; status: string };
...
export default function EntryDetailPage() {
  ...
  const [form, setForm] = useState({ date: "", classification: "food", paymentMethod: "cash", discount: "0", notes: "" });
  ...
  async function load() {
    ...
    if (res.ok) {
      setEntry(data.entry);
      setItems(data.items);
      setForm({ date: data.entry.date, classification: data.entry.classification, paymentMethod: data.entry.paymentMethod, discount: (data.entry.discountPaisa / 100).toString(), notes: data.entry.notes || "" });
    } else setMsg(data.error);
  }
  useEffect(() => { load(); }, [id, entryId]);

  async function save() {
    const payload: Record<string, unknown> = { date: form.date, classification: form.classification, paymentMethod: form.paymentMethod, discount: parseFloat(form.discount) || 0, notes: form.notes };
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-4">
      ...
      <h1 className="text-lg font-bold">এন্ট্রি {entry.id.slice(0, 8)} — {entry.date}</h1>
      <div className="bg-white border rounded-2xl p-4 space-y-2 text-sm">
        ...
        <div className="flex justify-between font-semibold"><span>সর্বমোট</span><span>{formatCurrency(entry.finalPaisa, locale)}</span></div>
        ...
        {!editing ? (
          ...
          <div className="space-y-3">
            <div className="grid md:grid-cols-2 gap-3">
              <div><label className="text-xs">তারিখ</label><input type="date" value={form.date} onChange={(e) => setForm({ ...form, date: e.target.value })} className="w-full border rounded-xl px-3 py-2 text-sm mt-1" /></div>
