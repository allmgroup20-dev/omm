...
export default function MatrixPage() {
  ...
  return (
    <div className="space-y-4">
      <Link href={`/messes/${id}/meals`} className="text-sm text-zinc-500">← Daily Entry</Link>
      <div className="flex items-center gap-3">
        <h1 className="text-lg font-bold">Monthly Matrix</h1>
      ...
      {!data ? (
        <div className="bg-white border rounded-xl p-6 text-sm">লোড হচ্ছে...</div>
      ) : (
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="bg-zinc-50">
                  <th className="text-left p-2 sticky left-0 bg-zinc-50">Member</th>
                  {data.dates.map((d) => (
                    <th key={d} className="p-2 text-center font-mono">{d.slice(8, 10)}</th>
                  ))}
              ...
              <tbody>
                {data.members.map((m) => (
                  <tr key={m.memberId} className="border-t">
                    ...
                    {data.dates.map((d) => (
                      <td key={d} className="p-2 text-center">{data.matrix[m.memberId]?.[d] ? data.matrix[m.memberId][d] / 100 : "-"}</td>
                    ))}
                    <td className="p-2 text-center font-bold">{(data.memberTotals[m.memberId] || 0) / 100}</td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
                <tr className="bg-zinc-900 text-white font-bold">
                  ...
                  {data.dates.map((d) => {
                    ...
                    return <td key={d} className="p-2 text-center">{dayTotal / 100 || "-"}</td>;
                  })}
                  <td className="p-2 text-center">{data.totalMealsScaled / 100}</td>
                </tr>
              </tfoot>
            </table>
          </div>
          <p className="p-3 text-xs text-zinc-500">Single source: meal_records → all pages show same totals.</p>
        </div>
      )}
    </div>
  );
